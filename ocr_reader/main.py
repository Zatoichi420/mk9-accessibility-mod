"""
Mortal Kombat Komplete Edition (MK9) accessibility reader (Phase 1 baseline).
...
This is ported near-verbatim from the MKX accessibility project's
ocr_reader/main.py (itself ported from MK Legacy Kollection) - same
architecture, same hardened reliability patterns (NVDA startup retry,
speak() failure self-heal, PID lock, poll-failure exit threshold, full-
desktop capture rather than PrintWindow - PrintWindow silently fails once
a game goes fullscreen, a real bug hit and fixed on the MKX project). See
that project's PROGRESS.md for the reasoning behind each of these choices;
only the game-specific bits (process name, highlight-color calibration)
differ here, and the latter is UNCALIBRATED - see the NEEDS_CALIBRATION
comment below. Unlike MKX, this game shipped with no PDB (see this
project's own PROGRESS.md), so there is no Phase 2 memory-reading
shortcut on the table yet - this OCR/library baseline is the plan, not a
placeholder for something faster.

known_screens/ starts empty for this project - nobody has captured and
hand-verified any MK9 screens yet (see PROGRESS.md's resume point). Until
...
# Per Phase 0 static analysis + multi-year community Cheat Engine/trainer
# precedent (see PROGRESS.md) - MKKE.exe, not MKLauncher.exe, is the real
# game. NOT yet confirmed via an actual live launch on this machine (unlike
# MKX's PROCESS_NAME, which was) - if the reader never finds its window,
# check this first.
PROCESS_NAME = "MKKE.exe"
...
# NEEDS_CALIBRATION: these values trace back to the MK Legacy Kollection
# project (calibrated for that launcher's cyan-selected/gold-unselected
# style), carried forward unchanged through MKX and now here purely as a
# starting point - none of these three games' UIs have ever been confirmed
# to share a highlight color scheme, and MK9's has NOT been sampled at
# all. Capture a real MK9 menu screenshot, sample the selected vs.
# unselected text pixel colors, and update these before trusting highlight
# detection in this game. Until then, is_line_highlighted() will likely
# just never fire (safe failure mode: no highlight spoken, not a wrong
# one).
